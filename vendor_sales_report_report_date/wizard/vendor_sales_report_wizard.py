...
class VendorSalesReport(models.TransientModel):
    ...
    excluded_vendor_ids = fields.Many2many('res.partner', string='Excluded Vendors',default=lambda self: self.env['res.partner'].search([('auto_excluded', '=', True)]))
    ...
    def _prepare_report_data(self):
        ...
        if self.filter_by == 'order_date':
            ...
            sale_domain = [("date_order", ">=", start_date), ("date_order", "<=", end_date)]
            move_domain = [("date", ">=", start_date), ("date", "<=", end_date)]

        else:
            start_date = self.from_report_date
            end_date = self.to_report_date
            sale_domain = [("report_date", ">=", start_date), ("report_date", "<=", end_date)]
            move_domain = [("report_date", ">=", start_date), ("report_date", "<=", end_date)]
        ...
        if suppliers_for_mail:
            ...
            if products_list:
                move_domain+=[("product_id", "in", products_list)]
            stock_lines = self.env['stock.move.line'].sudo().search(move_domain)
            ...
            if stock_lines:
                ...
                ############################################################################################################
                for invoice in sale_orders.invoice_ids.invoice_line_ids.filtered(
                        lambda o: o.move_id.state != 'cancel' and o.product_id.id in products_list and o.move_id.move_type == 'out_invoice' and o.move_id.invoice_date >= self.from_date and o.move_id.invoice_date <= self.to_date and o.move_id.state != 'cancel'):
                    lot_str = False
                    total_qty = 0
                    # this code for select the lot from Delivery according to the same quantity
                    for line in invoice.sale_line_ids.order_id.picking_ids.move_line_ids_without_package.filtered(
                    ...
                    if not lot_str:
                        lot_str = False
                        total_qty = 0
                        for picking in invoice.sale_line_ids.order_id.picking_ids.filtered(
                            ...
                            for line in picking.move_line_ids_without_package:
                                if line.product_id == invoice.product_id:
                                    lot_ids = invoice.sale_line_ids.order_id.picking_ids.move_line_ids_without_package.filtered(
                                        lambda
                                            o: o.product_id == invoice.product_id and o.picking_id.id == line.picking_id.id).mapped(
                                        'lot_id')
                                    if lot_ids:
                    ...
                    if invoice:
                        vendor_based_data = {
                            ...
                            "invoice_date": invoice.report_date if invoice else "",
                ...
                ############################################################################################################
                for invoice in sale_orders.invoice_ids.invoice_line_ids.filtered(
                        lambda o: o.move_id.state != 'cancel' and o.product_id.id in products_list and o.move_id.move_type == 'out_refund' and o.move_id.invoice_date >= self.from_date and o.move_id.invoice_date <= self.to_date):
                    lot_str = False
                    # this code for select the lot from Delivery according to the same quantity
                    for line in invoice.sale_line_ids.order_id.picking_ids.move_line_ids_without_package.filtered(
